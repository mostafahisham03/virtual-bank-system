    ...
    public Transaction executeTransfer(UUID transactionId) {
        ...
        Transaction transaction = transactionRepository.findByIdAndStatus(transactionId, TransactionStatus.INITIATED)
                .orElseThrow(() -> new IllegalArgumentException(
                        "Transaction not found or not in 'INITIATED' status with ID: " + transactionId));
        ...
        try {
            ...
            webClient.put()
                    ...
                    .onStatus(status -> status.is4xxClientError() || status.is5xxServerError(),
                            clientResponse -> clientResponse.bodyToMono(String.class) // Extracts the error body as a
                                                                                      // String.
                                    .flatMap(errorBody -> { // Processes the error body.
                                        System.err.println("Account Service returned an error ("
                                        ...
                                        String reasonPhrase = (httpStatus != null) ? httpStatus.getReasonPhrase()
                                                : "Unknown Error";
                                        // FIX ENDS HERE

                                        // Throws a WebClientResponseException, which is a more specific and useful
                                        ...
                                        return Mono.error(new WebClientResponseException(
                                                ...
                                                null, null, null // Other headers/body details can be populated here if
                                                                 // needed.
                                        ));
                                    }))
                    .bodyToMono(Void.class) // Expects no response body on a successful call (e.g., HTTP 200 OK).
                    .block(); // Blocks the execution until the HTTP call completes. In a fully reactive
            ...
            System.err.println("An unexpected error occurred during transaction execution for ID " + transactionId
                    + ": " + e.getMessage());
            // Throws a generic RuntimeException.
            throw new RuntimeException("An unexpected error occurred during transaction execution: " + e.getMessage(),
                    e);
        }
