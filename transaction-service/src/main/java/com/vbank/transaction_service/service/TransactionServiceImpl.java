...
@Service // This annotation tells Spring that this class is a service component, responsible for business logic.
@RequiredArgsConstructor // Lombok generates a constructor that injects all 'final' fields (dependencies) automatically.
public class TransactionServiceImpl implements TransactionService { // Implements the TransactionService interface, defining its contract.

    private final TransactionRepository transactionRepository; // Dependency: Used to interact with the database for Transaction entities.
    private final WebClient.Builder webClientBuilder; // Dependency: Used to build instances of WebClient for making HTTP calls to other services.
    private final KafkaTemplate<String, String> kafkaTemplate; // Dependency: Used to send messages (logs) to Kafka topics.
    private final ObjectMapper objectMapper; // Dependency: Used to convert Java objects to JSON strings (for logging).

    // Injects the Kafka topic name for logging from application.properties, with a default value.
    ...
    @Transactional // This annotation ensures that the method runs within a database transaction. If any part of the method fails (throws an exception), all changes made to the database within this method will be rolled back.
    public Transaction initiateTransfer(UUID fromAccountId, UUID toAccountId, BigDecimal amount, String description) {
        // Validates that the transfer amount is positive. Throws an IllegalArgumentException if not.
        if (amount.compareTo(BigDecimal.ZERO) <= 0) {
            throw new IllegalArgumentException("Transfer amount must be positive.");
        }

        // Uses Lombok's @Builder to create a new Transaction object. This is a clean way to instantiate objects.
        Transaction transaction = Transaction.builder()
                ...
                .status(TransactionStatus.INITIATED) // Sets the initial status of the transaction to INITIATED.
                .timestamp(Instant.now())     // Sets the timestamp to the current UTC time.
                .build();                     // Builds the Transaction object.

        // Saves the newly created transaction to the database. This persists the 'INITIATED' state.
        Transaction savedTransaction = transactionRepository.save(transaction);
        // Logs the initiation request to Kafka, sending the entire saved transaction object as payload.
        logToKafka("Request_Initiation", savedTransaction);
    ...
    public Transaction executeTransfer(UUID transactionId) {
        // Retrieves the transaction from the database by its ID, ensuring it's in the 'INITIATED' status.
        // If not found or status is not 'INITIATED', it throws an IllegalArgumentException.
        Transaction transaction = transactionRepository.findByIdAndStatus(transactionId, TransactionStatus.INITIATED)
                .orElseThrow(() -> new IllegalArgumentException("Transaction not found or not in 'INITIATED' status with ID: " + transactionId));
        ...
        try {
            ...
            // Creates a Data Transfer Object (DTO) containing the necessary information for the Account Service.
            ...
            logToKafka("Request_AccountService_Call", updateDto);
            ...
            webClient.put()
                    ...
                    // Handles HTTP 4xx (client error) or 5xx (server error) responses from the Account Service.
                    .onStatus(status -> status.is4xxClientError() || status.is5xxServerError(),
                            clientResponse -> clientResponse.bodyToMono(String.class) // Extracts the error body as a String.
                                    .flatMap(errorBody -> { // Processes the error body.
                                        System.err.println("Account Service returned an error (" + clientResponse.statusCode() + "): " + errorBody);

                                        // FIX STARTS HERE
                                        // Get the integer status code
                                        int statusCodeValue = clientResponse.statusCode().value();
                                        // Resolve to HttpStatus enum (if possible) to get the reason phrase
                                        HttpStatus httpStatus = HttpStatus.resolve(statusCodeValue);
                                        // Use the reason phrase if resolved, otherwise a default
                                        String reasonPhrase = (httpStatus != null) ? httpStatus.getReasonPhrase() : "Unknown Error";
                                        // FIX ENDS HERE

                                        // Throws a WebClientResponseException, which is a more specific and useful exception for HTTP errors.
                                        return Mono.error(new WebClientResponseException(
                                                "Account Service call failed: " + errorBody, // Detailed error message.
                                                statusCodeValue,                            // HTTP status code (e.g., 400).
                                                reasonPhrase,                               // Use the resolved reason phrase here
                                                null, null, null // Other headers/body details can be populated here if needed.
                                        ));
                                    }))
                    .bodyToMono(Void.class) // Expects no response body on a successful call (e.g., HTTP 200 OK).
                    .block(); // Blocks the execution until the HTTP call completes. In a fully reactive system, you'd chain reactive operations here instead of blocking.
            ...
            transactionRepository.save(transaction); // Saves the updated transaction status to the database.
            logToKafka("Response_Execution_Success", transaction); // Logs the successful execution.
            return transaction; // Returns the successfully executed transaction.

        } catch (WebClientResponseException e) {
            // Catches exceptions specifically from WebClient HTTP errors (e.g., 400 from Account Service).
            ...
            logToKafka("Response_Execution_Failed_WebClient", e.getResponseBodyAsString() != null ? e.getResponseBodyAsString() : e.getMessage());
            System.err.println("Transaction execution failed for ID " + transactionId + " due to Account Service error: " + e.getMessage());
            // Re-throws an IllegalArgumentException, passing the root cause for better error propagation.
            ...
            transaction.setTimestamp(Instant.now()); // Updates timestamp.
            transactionRepository.save(transaction); // Saves the FAILED status to the database.
            logToKafka("Response_Execution_Failed_Unexpected", e.getMessage()); // Logs the unexpected failure.
        ...
        // No 'finally' block for saving status is needed here because save(transaction) is handled in both success and catch blocks.
    }

    @Override // Indicates that this method implements a method from the TransactionService interface.
    public List<Transaction> getTransactionsForAccount(UUID accountId) {
        // Retrieves a list of transactions where the given account is either the 'from' or 'to' account,
     ...
     * The `payload` object will be serialized to JSON and included as the 'message' field in the Kafka log.
     * This provides richer log data than just a simple string, making it easier for log aggregation systems to parse.
     * @param messageType A string indicating the type of log message (e.g., "Request_Initiation", "Response_Execution_Success").
     * @param payload The Java object to be serialized into the 'message' field of the Kafka log.
     */
    private void logToKafka(String messageType, Object payload) {
        try {
            ...
            // Constructs the final log message string in JSON format, embedding the serialized payload.
            ...
            // This is important so the main application logic isn't interrupted by logging failures.
     ...
     * This is crucial when embedding one JSON string (e.g., `jsonPayload`) inside another JSON string (the `logMessage`).
