...
public class LogConsumer {
    ...
    // @RequiredArgsConstructor from Lombok handles constructor injection for final
    // fields
    // We add a public constructor to ensure ObjectMapper is configured correctly if
    // not already.
    public LogConsumer(LogService logService, ObjectMapper objectMapper) {
        this.logService = logService;
        this.objectMapper = objectMapper;
        // Ensure ObjectMapper can handle Java 8 Date/Time types if not already
        // configured globally
    ...
    public void consumeLogMessage(String logMessageJson) {
        ...
        try {
            ...
            log.error("Error parsing JSON log message from Kafka: {}. Message content: {}", e.getMessage(),
                    logMessageJson);
